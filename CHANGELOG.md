...
- **Symfony Console 8, PHPUnit 12 and a quieter SDK.** Widening the matrix turned up three things that had nothing to do with this release: Console 8 removed `Application::add()`, PHPUnit 12 stopped reading `@dataProvider` docblocks, and the SDK printed a line per object built outside a booted app (fixed in SuperAgent 1.2.1, which this release pins). All three are handled across every supported major rather than by picking one.
...
- **The new frontier models are selectable everywhere.** `claude-fable-5-1` (Anthropic's most capable widely released model, and now what `fable` means), `gpt-6-astra` (OpenAI's new flagship), `gemini-3.8-flash`, `deepseek-flash` (V4.1 Flash), `qwen3.8-max-0902`, `glm-5.3` and `glm-5.3-flash`. The Claude / Gemini / Codex pickers pick them up from the SDK catalog automatically; the SuperAgent engine list names them explicitly so they show up offline too.
- **Your cost dashboard was showing wrong numbers.** Several rates changed under us and `model_pricing` now matches reality: **Sonnet 5 is $2/$10** (the launch intro rate became permanent — the increase to $3/$15 scheduled for 2026-09-01 was cancelled), the whole **GPT-5.6 line was marked down** at the Astra launch (Sol $5/$30 → $4/$20, Terra $2.50/$15 → $2/$12, Luna $1/$6 → $0.20/$1.20), **DeepSeek moved to peak/off-peak billing** (V4-Pro $0.435/$0.87 → $0.66/$1.98 off-peak base), and **Grok 4.5's cached-input rate was carrying 4.6's** ($0.50 → $0.30). New rows land for GPT-6 Astra, GPT-5.5, Fable 5.1, Gemini 3.7/3.8, the Qwen 3.8 tiers, GLM-5.3 and Grok 4.6.
- **`deepseek-v4-flash` is retired upstream.** DeepSeek still routes the id to V4.1 Flash, so nothing breaks — but anywhere this package named it (the squad "easy" tier, the fill-in-middle service) now names `deepseek-flash`. If your own config pins the old id, switch it while it still routes.
- **Meta's Muse Spark is now a first-class provider — two of them.** Add a provider of type **`meta`** for one-shot calls, or **`meta-responses`** for agent loops: the Responses route is the only one that carries the model's reasoning *across* turns, and it can run a turn in the background — submit it, poll it, fetch the result later — so a job that takes many minutes no longer needs a connection held open. Both read `META_API_KEY` (or `MODEL_API_KEY`, the name Meta's own docs use). Muse Spark takes text, images, video, audio and PDFs, with a 1M-token context, at $1.25 in / $4.25 out per 1M.
  - Meta also speaks an Anthropic-compatible protocol — no new provider type needed, just point an `anthropic-proxy` row at `https://api.meta.ai`.
  - The cheaper `-contributor` models are priced in the config for hosts that want them, but nothing routes there by accident: they cost ~12× less **because Meta trains on your prompts and completions**, so you have to name the id.
- **The squad "expert" tier stays on Opus 5.** SuperAgent now puts Fable 5.1 there; this package keeps Opus 5 because it costs a quarter as much. Set `squad.tier_map.expert` to `claude-fable-5-1` if you want the frontier tier.
- `superaicore --version` reports `1.1.15` — the version number now tracks the SuperAgent release it pins, so `superaicore 1.1.15` means "built against SuperAgent 1.1.15". 1.1.13 and 1.1.14 are skipped deliberately.

## [1.1.12] — 2026-08-14

**SuperAgent SDK 1.1.11 — the deepseek-harness wave rides along (spill seam, lossless compaction, OS-level bash sandbox), five provider flagships refresh (Grok 4.6, DeepSeek V4 Pro GA, Qwen3.8-Max, Gemini 3.7 Flash, GLM-5.3) — plus housekeeping: the SDK becomes the single source of truth for the tracing classes, copy-paste gets a CI gate, and the SmartFlow overlap with the SDK is documented as intentional.** SDK pin moves `^1.1.10` → `^1.1.11`. Upgrading is `composer update forgeomni/superaicore` — no migrations. Re-publish the config if you want the new pricing rows.

- **Big tool output stops being destroyed.** The SDK now persists tool output above ~30K chars to session-private storage instead of truncating it; the model gets a head/tail preview plus a `spill://` locator it can read back in chunks via the new `spill_read` tool. **Compaction becomes lossless** the same way: content removed by every compactor is recorded in a shadow store and stays retrievable via the new `session_query` tool. Both work through the `superagent` backend automatically — SuperAICore now forwards your dispatch's `session_id` (falling back to `metadata.session_id`) so spilled and shadowed content is keyed to the right session, and a per-call `disable_spill: true` opts a single dispatch out. Tune or disable via the SDK's `SUPERAGENT_SPILL*` / `SUPERAGENT_SHADOW*` envs.
- **Bash can be confined at the OS level.** The SDK gains a sandbox seam — macOS Seatbelt (`sandbox-exec`, deny-write-by-default) and Linux bubblewrap — behind `SUPERAGENT_SANDBOX=off|auto|require` (default `off`; `require` is fail-closed). No SuperAICore wiring needed; it applies wherever the SDK's bash tool runs.
- **Honesty purge upstream:** the SDK deleted 20 registered placeholder tools that returned `status: simulated` without doing anything. SuperAICore never referenced them — nothing to migrate.
- **New models routed and priced** (all reachable through the `superagent` backend and alias routing; say the short name and get the new flagship):
  - **`grok` now means Grok 4.6** (released 2026-08-12) — same $2 in / $0.50 cached / $6 out per 1M as 4.5, 500K context, and a new four-level effort dial (`low|medium|high|xhigh`). Grok 4.5 stays reachable by id; its cached-input price drops to $0.30.
  - **DeepSeek V4 Pro GA + Flash 0731** — both gain a genuine `low` reasoning-effort tier. Pricing moves to DeepSeek's peak/off-peak model (effective 2026-08-16): off-peak base Pro $0.66/$1.98 (cache-hit $0.022), Flash $0.22/$0.66 (cache-hit $0.007). Peak hours (01-04 + 06-10 UTC) bill 2× and are not modelled — override upward if your traffic is peak-heavy.
  - **`qwen` now means Qwen3.8-Max** (GA 2026-08-03) — multimodal (text+vision), 1M context, at $2/$6 (down from 3.7-Max's $2.50/$7.50). 3.7-Max stays reachable by id.
  - **`gemini` now means Gemini 3.7 Flash** (GA 2026-08-13) — priced at the intro rate $0.75 in / $0.075 cached / $3.75 out through 2026-12-31 (standard $1.50/$7.50 from 2027; bump the row then). 3.5 Flash stays served.
  - **GLM-5.3** (released 2026-08-14) — reachable by id (`glm-5.3`, 1M route `glm-5.3[1m]`); its API pricing is unpublished, so cost rows provisionally bill at the 5.2 rate ($1.40/$4.40) — same as the SDK. `glm` still defaults to 5.2 until real pricing lands.
- **Security: Guzzle floor raised to `^7.15.3`** (mirrors the SDK), clearing CVE-2026-69246 / CVE-2026-69245.

Housekeeping in the same release, inspired by [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness)'s engineering discipline (clone detection in CI, no silent duplicate implementations):
...
- **Copilot 1.0.71 — model catalog was two generations stale, and cost rows never matched.** The pickers now mirror the "Copilot CLI" column of GitHub's own supported-models table: **Claude Sonnet 5 / 4.6 / 4.5, Claude Fable 5, Claude Opus 4.8 / 4.7 / 4.6 / 4.5, Claude Haiku 4.5, GPT-5.6 (Sol / Luna / Terra), GPT-5.5 / 5.4 / 5.4-mini / 5.3-codex / 5-mini, Gemini 3.5 Flash & 3.1 Pro, Kimi K2.7 Code and MAI-Code-1 Flash**. Retired upstream and removed here: gpt-4.1, gpt-5, gpt-5.1 (dead since 2026-04-15 — asking for it now degrades to the current GPT default instead of erroring), gpt-5.1-codex*, gemini-3-pro-preview. Cost attribution was silently dead for every copilot run — the config keyed models with dashes while the wire reports dots (`claude-sonnet-4.6`); all rows now match. `COPILOT_HOME` is honored for login detection and MCP sync, headless dispatches pin `COPILOT_AUTO_UPDATE=false` so the CLI can't self-update mid-run, and `cli:install copilot --via=brew` works on macOS.
- **Cursor 2026.07.16 — upstream renamed the Grok tiers; ours pointed at a dead slug.** `grok` now maps to `cursor-grok-4.5-high` (the old `grok-4.5-xhigh` no longer exists — the CLI rejects it), old saved configs still resolve, and historical usage rows keep their pricing. Login detection stops counting a bare `~/.cursor` folder (the IDE creates it) as "logged in" — only the CLI's own state file counts. Streaming/JSON parsing re-verified unchanged against live runs of the full 193-model list.
- **Kiro 2.13.0 — Opus is gone upstream; our fallback list and tests still pinned it.** The model list now mirrors the live nine (sonnet-4.5/4/haiku-4.5, deepseek-3.2, minimax-m2.5/m2.1, glm-5, qwen3-coder-next, auto); asking for a dropped family passes through fail-closed so you see Kiro's real error instead of a silent substitute. Doc comments now point at the real MCP config path (`~/.kiro/settings/mcp.json`). And Kiro finally joins `cli:install`: `superaicore cli:install kiro` runs the official installer (brew cask fallback via `--via=brew`).
- **Kimi 1.49 — dispatch needed zero changes, but your skills never reached the legacy CLI.** The headless contract (variant probe, flags, stream-json) re-verified at source level against the installed 1.49 package. Real fix: the legacy-variant skill bridge wrote a digest file the CLI never reads; skills now install natively into `~/.kimi/skills/` (same SKILL.md format), and the inert digest is cleaned up once ownership is confirmed.



**Antigravity CLI arrives, and every other CLI engine is re-verified against what's actually installed.** Google retired gemini-cli's consumer tiers on 2026-06-18 (personal OAuth now fails with *IneligibleTierError*); its official successor — the Antigravity CLI (`agy`) — is now a first-class SuperAICore engine, and a live audit of claude / codex / gemini / grok against their current releases fixed several silent breakages. No SDK bump, no migrations.

- **New engine: Antigravity CLI (`antigravity_cli`)** — dispatch to `agy` everywhere the other CLIs work: `superaicore send antigravity "..."` (alias `agy`), engine pickers, `cli:install antigravity`, login detection, and every fallback chain (it rides directly behind `gemini_cli`, so tasks whose gemini OAuth died fall through to the same models via your Google subscription at $0/token). One subscription routes **Gemini 3.5 Flash / 3.1 Pro, Claude Sonnet & Opus 4.6, and GPT-OSS 120B** — say `flash`, `pro`, `sonnet`, `opus` or the usual slugs and the resolver maps them to what `agy --model` really accepts (it only takes display names, and feeding it anything else would have returned the *model list* as your "answer").
- **Grok 0.2.103 broke our result parsing — fixed.** The CLI renamed the answer field and reshaped its streaming events, so grok runs silently reported **zero tokens, turns and thinking**; all three wire generations now parse, stop reasons are normalized, and the envelope reports the actually-routed SKU (`grok-4.5-build`). Login detection also stops treating a mere `~/.grok` directory as "logged in" (it now checks the real credential file), and the binary is found even when `~/.local/bin` symlinks are missing.
- **Claude MCP sync actually reaches Claude now.** It used to write `mcpServers` into `~/.claude/settings.json` — a key Claude Code never reads — so every sync was a silent no-op; it now targets `~/.claude.json`, the file `claude mcp add -s user` really uses. Also fixed: passing `permission_mode: default` (removed in claude 2.1.x — it used to kill the whole run; invalid modes are now dropped with a warning), four new session env vars are scrubbed before spawning child claudes, and a nonexistent Sonnet id (`…-20241022`) was purged from the pickers.
- **Gemini 0.51 support** — streaming chat returned an **empty string** on every modern gemini (the response field became a plain string); fixed. Headless runs in untrusted folders now pass `--skip-trust` (0.51 silently downgrades `--yolo` there, stalling every tool call), skills stop being exposed twice on builds with the native skill protocol, cost attribution skips the flash-lite router side-call, and a warning fires when a provided API key would be silently ignored in favor of the machine's OAuth login. **Heads-up:** personal-OAuth gemini logins are dead upstream — use an API key or the new Antigravity engine.
- **Codex refresh** — stale model lists replaced with the current GPT-5.6 generation (`gpt-5.6-sol` default), the brew source became the correct `brew install --cask codex`, and the JSONL wire format was re-verified unchanged on 0.144.6.
