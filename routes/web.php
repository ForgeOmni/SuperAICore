...
/**
 * Each section below registers only when its group is switched on (1.2.0+).
 * The defaults reproduce the single `route.enabled` flag this file used to
 * have, so nothing changes for a host that sets none of them; a host that
 * wants the dispatcher but not, say, the OpenAI-compatible proxy or the PTY
 * endpoints can now say so. See SuperAICore\Support\RouteGroups.
 */
$group = static function (string $name, callable $routes): void {
    if (RouteGroups::enabled($name)) {
        $routes();
    }
};
...
$group('usage', function () {
    // ─── Usage ───
    Route::get('usage', [UsageController::class, 'index'])->name('usage.index');
});

$group('costs', function () {
    // ─── Cost Analytics ───
