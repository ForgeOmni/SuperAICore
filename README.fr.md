...
- **Sept moteurs d'exécution** — Claude Code CLI, Codex CLI, Gemini CLI, GitHub Copilot CLI, AWS Kiro CLI, Moonshot Kimi Code CLI et SuperAgent SDK — unifiés derrière un même contrat `Dispatcher`. Chaque moteur accepte un jeu fixe de types de provider :
  ...
  - **Moonshot Kimi Code CLI** (0.6.8+) : `builtin` (`kimi login` OAuth via `auth.kimi.com`). Septième moteur ; complète le `KimiProvider` HTTP direct du SDK en couvrant le chemin agentic-loop sur abonnement OAuth, à l'image du split `anthropic_api` ↔ `claude_cli`. Le fanout `Agent` natif est honoré par défaut (`AgentSpawn\Pipeline` se retire aussitôt, même posture que Claude / Kiro) ; basculez vers la Pipeline AICore à trois phases via `use_native_agents=false` quand vous avez besoin d'observabilité streaming par enfant ou de charges > 500 étapes. **Facturation par abonnement** — forfaits Moonshot Pro / Power.
  - **SuperAgent SDK** : `anthropic`, `anthropic-proxy`, `openai`, `openai-compatible`
- Les moteurs se déploient en interne sur dix adaptateurs Dispatcher (`claude_cli`, `codex_cli`, `gemini_cli`, `copilot_cli`, `kiro_cli`, `kimi_cli`, `superagent`, `anthropic_api`, `openai_api`, `gemini_api`) — adaptateur CLI quand le provider est `builtin` / `kiro-api`, adaptateur HTTP quand il utilise une clé API. Détail d'implémentation ; tous les noms restent adressables depuis la CLI si besoin.
...
- **`Runner\TaskRunner` — exécution de tâche en un appel** (0.6.6+) — wrapper de `Dispatcher::dispatch(['stream' => true, ...])` qui retourne un `TaskResultEnvelope` typé (success / output / summary / usage / cost / log file / spawn report). Remplace ~150 lignes de "build prompt → spawn → tee log → extract usage → wrap result" côté hôte par un seul appel. Identique pour les 6 CLI (claude / codex / gemini / kiro / copilot / kimi) — plus de branches par backend dans votre code. Voir `docs/task-runner-quickstart.md`.
- **`Contracts\StreamingBackend` — chaque CLI obtient tee live + Process Monitor + onChunk** (0.6.6+) — nouvelle interface sœur de `Backend::generate()` qui stream les chunks via un callback tout en les tee'ant sur disque et en enregistrant une ligne `ai_processes` pour l'UI Monitor. Les 6 backends CLI l'implémentent ; `Dispatcher::dispatch(['stream' => true, ...])` opt-in transparent. Honore les `timeout` / `idle_timeout` / `mcp_mode` par appel (`'empty'` pour claude empêche les MCP globaux de bloquer la sortie). Voir `docs/streaming-backends.md`.
...
- **Moonshot Kimi Code CLI — septième moteur d'exécution** (0.6.8+) — intégration complète du CLI `kimi` de Moonshot (vérifié v1.38.0) en tant que `kimi_cli` aux côtés des six backends existants. `KimiCliBackend` spawne `kimi --print --output-format=stream-json` et parse les trois shapes d'événements (`role=assistant` avec `content[].type=think|text` + `tool_calls[]` optionnels, `role=tool` portant `tool_call_id`, texte assistant final qui remplace les partials) ; les blocs `think` restent internes, jamais dans le texte de l'enveloppe. Max-steps-per-turn, fichier MCP par run, `cwd` circulent via `StreamingBackend`. Routage d'agent-teams au choix de l'opérateur : fanout `Agent` natif par défaut (preamble + consolidation renvoient vide, `AgentSpawn\Pipeline` se retire — même posture que Claude / Kiro), ou `use_native_agents=false` pour router via la Pipeline trois-phases d'AICore et hériter du durcissement modèles-faibles 0.6.8. Nouvelle commande `kimi:sync` traduit les listes d'outils de `.claude/agents/*.md` en `~/.kimi/agents/*.yaml` + `~/.kimi/mcp.json` (compatible Claude) ; `claude:mcp-sync` fan-out sur Kimi automatiquement. Complémentaire du `KimiProvider` HTTP direct du SDK pour couvrir le chemin agentic-loop sur abonnement OAuth, miroir du split `anthropic_api` ↔ `claude_cli`. Config sous `backends.kimi_cli.{enabled,binary,timeout,max_steps_per_turn,use_native_agents}`.
- **Sync MCP piloté par catalogue** (0.6.8+) — `claude:mcp-sync` lit `.mcp-servers/mcp-catalog.json` + un fin mapping `.claude/mcp-host.json` et distribue le bon sous-ensemble de serveurs au `.mcp.json` projet, aux blocs `mcpServers:` dans la frontmatter de chaque agent `.claude/agents/*.md` (entre les marqueurs `# superaicore:mcp:begin` / `:end`), et à la config user-scope de chaque backend CLI installé (Claude / Codex / Gemini / Copilot / Kiro / Kimi). `mcp:sync-backends` est le point d'entrée de propagation autonome pour un `.mcp.json` édité à la main ou une auto-sync par file-watcher. Non destructif : les fichiers édités par l'utilisateur sont repérés via un manifeste sha256 et laissés intacts (pour le fichier projet) ; les éditions dans la frontmatter d'agent hors marqueurs sont préservées. Voir `docs/mcp-sync.md`.
- **`SuperAgentBackend` promu en véritable boucle agentique** (0.6.8+) — honore `max_turns` (défaut 1, préserve le one-shot), `max_cost_usd` → `Agent::withMaxBudget()` pour plafond budget dur dans la loop, filtres `allowed_tools` / `denied_tools`, `mcp_config_file` (charge un `.mcp.json` via `MCPManager::loadFromJsonFile()` + `autoConnect()`, déconnecte en `finally{}`), et `provider_config.region` routé via `ProviderRegistry::createWithRegion()` pour le découpage régional Kimi / Qwen / GLM / MiniMax de SuperAgent 0.8.8. L'enveloppe gagne `usage.cache_read_input_tokens`, `usage.cache_creation_input_tokens`, `cost_usd` (coût auto-calculé par le SDK — Dispatcher préfère cette valeur) et `turns`. **SDK épinglé sur 0.9.0 (0.6.9+)** : quand un appelant active le dispatch sub-agent du SDK (`load_tools: ['agent', …]`), l'enveloppe ajoute une clé optionnelle `subagents` portant les infos productivity de l'`AgentTool` 0.8.9 (`filesWritten` / `toolCallsByName` / `productivityWarning` / `status: completed|completed_empty`) — utile pour détecter un sub-agent qui n'a produit que du texte sans scraper la narration. Omise quand aucun sub-agent n'a tourné, donc la shape reste byte-compatible.
...
- **SuperAgent SDK monté en 0.9.0** (0.6.9+) — Contrainte Composer remontée `^0.8.0` → `^0.9.0`. **Correctifs automatiques** (aucun changement de code requis) : la wire shape Kimi `thinking` (avant 0.9.0 le SDK envoyait un id de modèle fabriqué `kimi-k2-thinking-preview` que Moonshot n'a jamais publié — chaque appel thinking finissait en 400, maintenant le SDK envoie le vrai modèle + `reasoning_effort` + `thinking: {type: enabled}`) ; l'assemblage SSE des tool_calls fragmentés (un tool_call découpé en N chunks apparaissait comme N ContentBlocks — accumulation par index désormais, donc le compte `subagents[]` devient exact sur tous les providers OpenAI-compat) ; `finish_reason: "error_finish"` lève désormais un 429 re-tryable au lieu d'être silencieusement appendé au texte résultat ; les cached-tokens sont lus depuis les deux shapes `usage.prompt_tokens_details.cached_tokens` (shape OpenAI actuelle, émise par Kimi) et `usage.cached_tokens` (legacy) — `cache_read_input_tokens` devient exact sur Kimi alors qu'il était 0 avant ; le refresh OAuth Anthropic est sérialisé par `flock`, donc des workers queue Laravel partageant les credentials OAuth stockés n'écrasent plus mutuellement leurs refresh tokens.
- **`super-ai-core:models refresh [--provider <p>]`** (0.6.9+) — nouveau sous-commande aux côtés de `list / update / status / reset`. Enveloppe `ModelCatalogRefresher` du SDK 0.9.0 pour tirer l'endpoint `GET /models` live de chaque provider dans un cache overlay par-provider à `~/.superagent/models-cache/<provider>.json`. Supporte anthropic / openai / openrouter / kimi / glm / minimax / qwen. Overlay au-dessus de l'override utilisateur, en-dessous des `register()` runtime, donc les prix bundled sont préservés quand le `/models` du vendor omet les tarifs (ce qui est la règle). La sortie `status` gagne une ligne `refresh cache`.
- **`SuperAgentBackend` transmet trois options 0.9.0** (0.6.9+) — nouvelles clés additives sur `Dispatcher::dispatch()` quand `backend=superagent` : `extra_body` (deep-merge au niveau top du body de chaque requête `ChatCompletionsProvider` — échappatoire power-user pour champs wire spécifiques au vendor), `features` (routées par le `FeatureDispatcher` SDK ; clés utiles : `prompt_cache_key.session_id` pour le cache prompt session-level de Kimi, `thinking.*` avec fallback CoT gracieux sur chaque provider, `dashscope_cache_control` pour les marqueurs cache style Anthropic sur Qwen), et `loop_detection: true|array` (enveloppe le streaming handler dans `LoopDetectionHarness` — attrape `TOOL_LOOP` / `STAGNATION` / `FILE_READ_LOOP` / `CONTENT_LOOP` / `THOUGHT_LOOP`). Raccourci : `prompt_cache_key: '<sessionId>'` accepté directement comme shorthand session_id.
- **Support dual Qwen + reconnaissance OAuth Kimi/Qwen Code** (0.6.9+) — le SDK 0.9.0 rebinde la clé registry `qwen` sur un provider OpenAI-compat (`<region>/compatible-mode/v1/chat/completions` — c'est ce que le CLI `qwen-code` d'Alibaba utilise en production) et déplace l'ancienne wire shape DashScope native sur `qwen-native`. Les deux clés partagent `QWEN_API_KEY`. `ApiHealthDetector::DEFAULT_PROVIDERS` inclut maintenant `qwen-native` aux côtés de `qwen`, donc les hôtes qui ont encore besoin de `parameters.thinking_budget` / `parameters.enable_code_interpreter` voient les deux endpoints dans la sonde dashboard. `filterToConfigured()` traite en plus `~/.superagent/credentials/kimi-code.json` / `qwen-code.json` (écrits par `superagent auth login kimi-code` / `qwen-code`) comme « configurés », donc un opérateur qui s'est logué en OAuth sans clé API apparaît toujours dans `api:status` / `/providers`.
- **`McpManager` — helpers OAuth pour serveurs mcp.json** (0.6.9+) — `oauthStatus(key) → 'ok'|'needed'|'n/a'`, `oauthLogin(key)`, `oauthLogout(key)` — fins wrappers autour de `McpOAuth` du SDK 0.9.0 pour les serveurs MCP dont l'entrée mcp.json déclare un bloc `oauth: {client_id, device_endpoint, token_endpoint, scope?}`. Complémentaires des `startAuth()` / `clearAuth()` / `testConnection()` préexistants (qui gèrent les serveurs type browser-login / session-dir, ex. scraper LinkedIn). Une UI hôte peut maintenant afficher un bouton OAuth par serveur MCP dont la config déclare le device-code flow. Le login bloque sur stdio pendant le poll device-flow — à pousser en job queue, pas en ligne dans une requête.
