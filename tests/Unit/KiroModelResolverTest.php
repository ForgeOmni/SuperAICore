...
final class KiroModelResolverTest extends TestCase
{
     ...
     * json-pretty` (captured 2026-07-19 against Kiro CLI 2.13.0 — the
     * Opus SKUs and claude-sonnet-4.6 disappeared upstream in this
     * generation). Any field rename upstream will be caught by this
     * assertion before it reaches a production host.
     */
    ...
    public function test_parses_live_list_models_payload(): void
    {
        ...
        $this->assertContains('claude-sonnet-4.5', $slugs);
        ...
        $this->assertSame('sonnet',   $byId['claude-sonnet-4.5']['family']);
        ...
        $this->assertSame(
            ...
            $byId['claude-sonnet-4.5']['display_name']
        );
    }
    ...
    {
        $this->assertSame('claude-sonnet-4.5', KiroModelResolver::resolve('claude-sonnet-4-5'));
    ...
    {
        // Claude CLI emits `claude-sonnet-4-7[1m]` for its 1M-context
        // variant — Kiro has no 4.7, so we fall back to the family
        // default 4.5.
        $this->assertSame(
            'claude-sonnet-4.5',
            KiroModelResolver::resolve('claude-sonnet-4-7[1m]')
        );
        // `claude-sonnet-4-5-20241022` strips its date, dash→dot converts,
        // and `claude-sonnet-4.5` IS in Kiro's catalog.
        $this->assertSame(
            'claude-sonnet-4.5',
    ...
    {
        $this->assertSame('claude-sonnet-4.5', KiroModelResolver::resolve('sonnet'));
        $this->assertSame('claude-haiku-4.5',  KiroModelResolver::resolve('haiku'));
    }

    public function test_resolve_passes_dropped_opus_family_through(): void
    {
        // Kiro 2.13 removed every Opus SKU. We deliberately do NOT
        // substitute a different family — pass through and let kiro-cli
        // fail with its own error (fail closed, no silent model swap).
        $this->assertSame('opus',            KiroModelResolver::resolve('opus'));
