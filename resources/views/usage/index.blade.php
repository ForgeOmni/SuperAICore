        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body p-0">
                ...
                @foreach($byModel as $model => $d)
                    <tr><td class="small font-monospace">{{ $model }}</td><td class="text-end">{{ $d['runs'] }}</td><td class="text-end">${{ number_format($d['cost'], 4) }}</td></tr>
                @endforeach
                </tbody></table>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body p-0">
                ...
                @foreach($byBackend as $be => $d)
                    <tr><td>{{ $be }}</td><td class="text-end">{{ $d['runs'] }}</td><td class="text-end">${{ number_format($d['cost'], 4) }}</td></tr>
                @endforeach
                </tbody></table>
            </div>
        </div>
