...
- `superaicore skill:install-dispatch [--agent <a>|--agent all] [--uninstall]`
  ...
  `superaicore:sync-cli` 方向相反）。支持 claude、codex、gemini，1.1.5 起
  新增 grok、cursor（`~/.cursor/skills-cursor`）、qwen；默认只装 claude；
...
## 39. Antigravity CLI —— gemini-cli 继任者作为派单引擎（1.1.9）

Google 于 2026-06-18 停掉了 gemini-cli 的个人档位:个人 OAuth 运行在上游
直接报 `IneligibleTierError`(exit 55),重新登录也无济于事。替代品是
Antigravity CLI(`agy`,Go 单二进制,实测 1.1.4)—— 1.1.9 把它做成一等
引擎 `antigravity_cli`。
...
$dispatcher->dispatch([
    ...
    'prompt'  => '把这个方法重构得更清晰。',
    'model'   => 'flash',            // 或 'pro' / 'sonnet' / 'opus' / slug
]);
```

CLI 侧:`superaicore send antigravity "..."`(别名 `agy`)。续聊用
`resume_session_id`(→ `--conversation <id>`)或 `continue_session`
(→ `--continue`)。

### 严格模型解析器

`agy --model` 只接受与 `agy models` 输出逐字一致的完整显示名
(`Gemini 3.5 Flash (Low)`)。其他输入的失败模式对自动化极不友好:agy
把模型列表打到 stdout 且退出码 0 —— 天真的后端会把它当"回答"返回。
因此 `AntigravityModelResolver` 是严格模式:族别名
(`flash`/`pro`/`sonnet`/`opus`/`gpt-oss`)和跨引擎 slug
(`gemini-3.5-flash-low`、`claude-opus-4-6`、`gpt-oss-120b` 等)映射到
显示名;未知输入解析为 null、直接丢弃 flag(走 agy 默认模型)。一个
Google 订阅路由 Gemini 3.5 Flash(三档)、Gemini 3.1 Pro(两档)、
Claude Sonnet/Opus 4.6、GPT-OSS 120B —— 本通道全部 $0/token。

### 认证与检测

登录仅限 TUI(不带参数跑 `agy`,Google OAuth)。凭证与 gemini-cli
**共享** `~/.gemini/oauth_creds.json`;agy 状态在
`~/.gemini/antigravity-cli/`。状态检测因此要求两者齐备(凭证 + 状态
目录)才报已登录 —— 只有凭证文件可能是死掉的 gemini-cli 残留。官方
安装器落 `~/.local/bin/agy`;`isAvailable()` 对非 login shell 直接探测
该路径。

### fallback 站位

`antigravity_cli` 在所有内置链(`chains_by_profile`、
`chains_by_metadata`、`auto_chain`)中紧跟 `gemini_cli`:在 gemini 已
退役 OAuth 上失败的任务,零配置落到 agy 订阅下的同款 Gemini 模型。

### 边界（v1）

print 模式是纯文本 —— 无 stream-json、无 token 用量(订阅本就计 $0)。
MCP/扩展管理走 `agy plugin`,尚无经验证的宿主可写配置文件,故
`supportsMcp()` 为 false、技能桥条目为 `none`。agy 自带 agent 编排
(spawn-plan 快速退出),与 Claude / Grok / Kiro 同策略。

## 40. 第二波 CLI 审计 —— copilot / cursor / kiro / kimi 真机对齐（1.1.10）

*自 1.1.10 起。无 SDK bump、无迁移、无新 config 键。*

1.1.9 审计了 claude / codex / gemini / grok / antigravity;本波对齐其余
四个引擎:copilot 1.0.71、cursor-agent 2026.07.16、kiro-cli 2.13.0、
kimi(legacy uv kimi-cli)1.49.0。逐项对照本机实装二进制(help 面、
`models` / `--list-models` 输出、wire 抓包、kimi 的 Python 包源码),
copilot 名录另对照 GitHub supported-models 参考页的 "Copilot CLI" 列
(实时 HTML 表格解析)。

### `cli:install` 扩容:kiro 上线,copilot 补 brew

```bash
./vendor/bin/superaicore cli:install kiro                 # 官方脚本(默认)
./vendor/bin/superaicore cli:install kiro --via=brew      # brew install --cask kiro-cli
./vendor/bin/superaicore cli:install copilot --via=brew   # brew install --cask copilot-cli
```

装完照旧:`kiro-cli login`(或 `KIRO_API_KEY` headless)、
`copilot login`。`cli:status` / `cli:install --all-missing` 自动纳入 kiro。

### Copilot:家族别名与退役降级

`CopilotModelResolver` 新增家族 `fable`(→ `claude-fable-5`)与 `mai`
(→ `mai-code-1-flash`);`opus` 默认升至 `claude-opus-4.8`,`gpt` 指向
`gpt-5.6-sol`(5.6 三 SKU:Sol / Luna / Terra)。已退役 slug 的行为值得
了解:`gpt-5.1`(2026-04-15 上游退役)不再报错,而是经
`familyFromName()` 降级到 gpt 家族默认 —— 派发日志与成本行会记录实际
路由的 slug。成本行全部换成 wire 真实上报的点号 id;两个新 env 行为:
登录检测与 MCP 同步识别 `COPILOT_HOME`,headless 派发注入
`COPILOT_AUTO_UPDATE=false`。

注意:模型可用性仍受订阅档位服务端限制(Free/Student 仅 Auto / mini
档);GPT-5.6 三 SKU 与 Opus 4.8 fast-mode(preview,无公开 slug)在
mini 档账号上无法 wire 实证 —— 名录以 GitHub 文档表为准,见
`CopilotModelResolver` 头注释的 slug caveat。

### Cursor:Grok 档位改名的兼容策略

上游把 `grok-4.5-{low,medium,high,xhigh}[-fast]` 改名为
`cursor-grok-4.5-*` 并删掉 xhigh 档。`CursorModelResolver` 的 `grok`
别名与目录行改指 `cursor-grok-4.5-high`;`familyFromName()` 兼容
`cursor-grok-*` 前缀,所以数据库里存过旧 slug 的偏好 / 别名配置无需
迁移,解析时自动落到现役档位。历史 `cursor:grok-4.5-xhigh` 计价 key
保留,旧用量行不丢价。登录检测改为只认
`~/.cursor/agent-cli-state.json`(CLI 自写文件)—— 裸 `~/.cursor`
目录是 Cursor IDE 的产物,不代表 CLI 登录过。

### Kiro:九模型 + fail-closed 透传

2.13.0 上游砍掉全部 Opus SKU 与 claude-sonnet-4.6 / opus-4.5;
`KiroModelResolver::STATIC_FALLBACK` 镜像实测九模型(auto、
claude-sonnet-4.5 / 4、claude-haiku-4.5、deepseek-3.2、minimax-m2.5 /
m2.1、glm-5、qwen3-coder-next)。请求已下架家族(如 `opus`)时
**fail-closed 透传** —— 你拿到 kiro-cli 的真实报错,而非静默换模型;
这与派单重试策略一致(只重试 quota / rate_limit / auth / network,
其余 fail closed)。MCP 配置路径以 `~/.kiro/settings/mcp.json` 为准。

### Kimi:legacy 技能桥从 no-op 变一等安装

源码级复验证实 legacy kimi-cli 1.49 原生发现 `~/.kimi/skills/`
(SKILL.md 包,与 `~/.claude/skills` 合并不遮蔽),而旧技能桥写的
`~/.kimi/super-team-skills.md` 摘要 CLI 从来不读。
`CliSkillBridge::descriptor('kimi')` 现在两代都返回 `native_dir`
(按代际选 `~/.kimi/skills/` 或 `~/.kimi-code/skills/`),
`superaicore:sync-cli` 逐技能落盘;一次性迁移 `pruneKimiDigest()` 在
