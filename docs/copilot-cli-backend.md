...
`--model=<model>` 启动时选，`/model <model>` 会话内切。支持模型（截至 2026-07，CLI 1.0.71）：

- Claude Sonnet 5、Claude Sonnet 4.6 / 4.5、Claude Fable 5、Claude Opus 4.8 / 4.7 / 4.6 / 4.5（另有 Opus 4.8 fast-mode preview，无公开 slug）、Claude Haiku 4.5
- GPT-5.6 Sol / Luna / Terra、GPT-5.5、GPT-5.4、GPT-5.4-Mini、GPT-5.3-Codex、GPT-5-Mini（GPT-5.1 已于 2026-04-15 退役，resolver 自动降级到 gpt 家族默认）
- Gemini 3.5 Flash、Gemini 3.1 Pro、Kimi K2.7 Code、MAI-Code-1-Flash（raptor-mini / gpt-5.4-nano / gemini-2.5-pro / gemini-3-flash 在文档表中标记为 CLI 不可用）
- 名录来源：GitHub supported-models 参考页的 "Copilot CLI" 列（2026-07-19 解析自实时表格）；CLI 自 1.0.6x 起原生接受 `sonnet` / `opus` / `haiku` / `gpt` / `gemini` 家族别名；模型可用性受订阅档位服务端限制（Free/Student 仅 Auto/mini 档）
