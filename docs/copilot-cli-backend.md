...
**状态**：设计中，尚未实现
**日期**：2026-04-18
**目标版本**：待定（不自动 bump，按当前 0.5.6 基线开发）
...
- [ ] `copilot:sync` 命令：扫描 `.claude/agents/*.md` → 翻译并写 `~/.copilot/agents/*.agent.md`
  - Front-matter 重排：`tools`（Claude 列表）→ Copilot `tools`（以 `--allow-tool` 表达式形式）
  - `model` 字段 Copilot 不支持，丢弃或写注释
  - `instructions` ≈ Claude 的正文 body
- [ ] `agent:run --backend=copilot` via `copilot --agent <name> -p ...`，若目标文件不存在提示用户先 `copilot:sync`
- [ ] `CopilotMcpSync`：复用 MCP manager，输出到 `~/.copilot/mcp-config.json`（注意与 Copilot 内建 GitHub MCP 共存，不要覆盖用户已有配置 —— 同 `ClaudeMcpSync` 的 merge 策略）
- [ ] `CopilotToolPermissions`：Claude tool 声明 → `--allow-tool=...` 字符串
- [ ] Pricing：`ai_model_pricing` 加 `billing_model` 列，Copilot 标 `subscription`
- [ ] Usage 日志：若 `--output-format=json` 可用，解析 JSONL 抽取 tokens；否则仅记时长与 exit code
- [ ] Dashboard：「Subscription engines」区块
