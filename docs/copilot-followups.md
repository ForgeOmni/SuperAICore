# Copilot CLI backend — deferred follow-ups
...
## 1. CLI usage extraction for Claude / Codex / Gemini  ★最高价值
...
## 6. Process registration cross-engine 推广
...
| # | 项目 | ROI | 风险 | 建议时机 |
|---|---|---|---|---|
| 1 | CLI usage extraction (Claude/Codex/Gemini) | 高 | 中 | **下一个 sprint** |
| 6 | Process registration 推广到其它 runner | 中 | 低 | 顺手做，<2 小时 |
| 7 | XDG 写入路径修复 | 低 | 低 | 等到有用户在 macOS / Linux 用 XDG 跑 Copilot 时 |
| 2 | Plugin-skill 警告文档（B 方案） | 低 | 零 | 加在下次文档更新 |
| 3 | `/fleet` 暴露 | 中 | 中 | 等 host app（如 SuperTeam）需要并行 fan-out |
| 4 | Hooks 对接 | 低 | 高 | 等用户提需求 |
